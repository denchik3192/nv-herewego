...
function Scene({
  ...
  const [isShowHotstopt, setIsShowHotstopt] = useState({
    front: true,
    middle: true,
    back: true,
  });
  ...
  useEffect(() => {
    if (!constainerRef.current || !originRef.current) return;
    gsap.to(constainerRef.current.position, {
      x: 0.2,
      duration: 3,
      repeat: -1,
      yoyo: true,
  ...
  function onClick(event: ThreeEvent<MouseEvent>) {
    ...
    setIsShowHotstopt((current) => ({ ...current, [name]: false }));
      ...
      <group ref={originRef}>
        <group ref={constainerRef} position={[-0.25, 0, -0.635]}>
          <group position={[0, -0.086, 0.635]}>
            ...
            <Hotspot
              isVisible={!animating && isShowHotstopt.front}
              position={[0, 0.38, 1]}
              color="#B8FC39"
            />
            ...
            <Hotspot
              isVisible={!animating && isShowHotstopt.middle}
              position={[0, 0.33, 0]}
              color="#FF7A51"
            />
            ...
            <Hotspot
              isVisible={!animating && isShowHotstopt.back}
              position={[0, 0.35, -0.9]}
              color="#46ACFA"
            />
