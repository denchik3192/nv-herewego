...
function Parallax({ foregroundImage, backgroundImage, className }: Props) {
  ...
  useEffect(() => {
    ...
    function animationFrame() {
      ...
      if (backgoundRef.current) {
        backgoundRef.current.style.transform = `translate(${newX}px, ${newY}px)`;
      }
  ...
  return (
    <div ref={backgoundRef} className={clsx('grid grid-cols-1 place-items-center', className)}>
      <div className="col-start-1 row-start-1 transition-transform">
        <PrismicNextImage field={backgroundImage} alt="" className="w-11/12" />
      </div>
      <div
        ref={foregroundRef}
        className="col-start-1 row-start-1 transition-transform w-full place-items-center">
        <PrismicNextImage
