...
export function Skateboard({
  ...
  truckColor,
  boltColor,
  constantWheelSpeed = false,
}: SkateboardProps) {
  const wheelRefs = useRef<THREE.Object3D[]>([]);

  const { nodes } = useGLTF("/skateboard.gltf") as GLTFResult;

  const whellTextures = useTexture(wheelTextureURLs);

  whellTextures.forEach((texture) => {
    texture.flipY = false;
  ...
  const wheelTexture = whellTextures[wheelTextureIndex];
  ...
  useFrame(() => {
    if (!wheelRefs.current || !constantWheelSpeed) return;
  ...
  }, [constantWheelSpeed, wheelTextureURL]);
